...
# Remove outliers from age variables
...
# Remove duplicates from cov file
sum(duplicated(cov$IID))
cov <- cov %>%
...
df <- df %>%
  mutate(height = findouts(height, upper_sd = 4, lower_sd = 4)) %>% # 18 removed as outliers  
  mutate(exam = findouts(exam, upper_sd = 4, lower_sd = 4)) # 1 outlier removed
...
sum(duplicated(df$IID))

df <- df %>%
  filter(!duplicated(df$IID))
...
# Save seperate covariates files for height/exam age at measurement 

cov_height <- cov %>%
  select(-Age_exam)

cov_exam <- cov %>%
  select(-Age_height,-Age_exam) # no variance in Age_exam covariate, remove from datafile 

write.table(cov_height, "N:/durable/projects/qc_paper/trio-GWAS/scratch/covariates_height.cov", col.names=T, row.names=F, quote=F, sep='\t')
write.table(cov_exam, "N:/durable/projects/qc_paper/trio-GWAS/scratch/covariates_exam.cov", col.names=T, row.names=F, quote=F, sep='\t')
