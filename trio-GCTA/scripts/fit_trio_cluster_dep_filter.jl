...
#filter GRM on relatedness and fit full, no covariance, direct only, and null models for smfq_dep phenotype
...
function sel_trio(A, m_inds, f_inds, c_inds)
    ...
    A_mf = A[mf_inds, mf_inds]
     # A_mf = view(A, mf_inds, mf_inds) # This view may be wrong and remove parent-offspring!
...
mobadat[!, :Age_dep] .= float.(mobadat[!, :Age_dep])
...
mf_pc = m_pc + f_pc
mf = ModelFrame(term(Symbol(vari)) ~ ConstantTerm(1) + term(:genotyping_batch_num) + term(:sex) + term(:Age_dep), mobadat[c_inds_sel_nomiss, :])
X = ModelMatrix(mf).m
X = hcat(X, mf_pc)
y = response(mf)
y = float(y)
