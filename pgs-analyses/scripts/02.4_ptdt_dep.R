...
# In this script we select trios on the basis of "extreme" (high) depressive symptom scores among children, 
# dropping those whose parents also score in the "extreme" on their own depressive symptoms, attaching siblings as comparators,
...
dat_ptdt <- dat_final %>% 
  filter(!is.na(smfq_dep),
         if_all(matches("pgs_res"), ~!is.na(.))) %>% 
  mutate(mdep = scale(scl_dep_m_q3),
         fdep = scale(scl_dep_f_far)) |> 
  group_by(m_id) %>%
  slice_sample(n=1)%>%
  rowwise() %>% 
  mutate(pdep = mean(c(mdep,fdep))) %>% 
  ungroup() %>% 
  mutate(ext_ch = ifelse(smfq_dep> (mean(smfq_dep,na.rm=T)+2*sd(smfq_dep,na.rm=T)), "Yes","No" ),
         ext_p = ifelse(pdep> (mean(pdep,na.rm=T)+2*sd(pdep,na.rm=T)), "Yes","No" )) %>% 
  filter(ext_ch=="Yes",
         ext_p=="No")
...
lem <- function(x){ 
  tmp <- lm(x~1+SEX+scale(Age_dep), data= ptdat)
...
save(ptdres, file = "./output/ptd_dep.RData")
...
p1<- ggplot(ptdres, aes(x=Group,y=mean,fill=Group,colour=Group)) +
  ...
  scale_x_discrete("PGS trait and group (probands ascertained\n for high depressive symptoms scores at age 8, their siblings)")
